...
uptime-monitor/
├── app/
│   ├── main.py          # точка входа FastAPI, lifespan запускает scheduler
│   ├── models.py        # SQLAlchemy модели: Monitor, CheckResult
│   ├── database.py      # подключение к БД
│   ├── schemas.py       # Pydantic схемы
│   ├── checker.py       # проверка доступности одного URL (httpx)
│   ├── scheduler.py     # фоновый APScheduler-джоб, проверяет активные мониторы
│   └── routers/
...
2. **Разделение на сервисы** — текущий этап. Выносим проверки в отдельный воркер, добавляем Redis как очередь задач.
